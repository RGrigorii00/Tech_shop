<template>
  <div>
    <h1 style="display: flex; justify-content: center; margin: 30px;">Список проданных товаров</h1>
    ...
    <button class="btn btn-primary mt-3" @click="print1">Сгенерировать PDF</button>
  ...
  computed: {
    printOrders() {
      const sellData = this.orders.map(order => ({
        ...
        user_name: order.sell_un_id || 'Неизвестно',
        product_id: order.sell_p_id,
        product_name: order.sell_pn_id || 'Неизвестно',
        quantity: order.sell_q_id,
        total_price: order.sell_tp_id,
        sale_date: order.autoCreateTime,
        cart_id: order.CartID || 1
      }));

      if (sellData.length === 0) {
        console.error('Нет данных для создания PDF');
        alert('Нет данных для создания PDF');
        return;
      }

      console.log("Отправляемые данные:", sellData);
    ...
    totalAmount() {
      ...
      return this.orders.reduce((sum, order) => {
        const value = Number(order.sell_tp_id);
          ...
          <table>
            ...
            <tbody>
              ...
              <tr class="total-row">
                ...
                <td class="total-cell">${this.formatNumber(this.totalFilteredAmount)} ₽</td>
                <td></td>
              </tr>
            </tbody>
          </table>
